...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  for (const field of ["name", "username", "email", "loginMethod", "passwordHash"] as const) if (user[field] !== undefined) { values[field] = user[field] ?? null; updateSet[field] = user[field] ?? null; }
...
export async function getUserByEmailOrUsername(identifier: string) {
  ...
  return (await db.select().from(users).where(or(eq(users.email, identifier.toLowerCase()), eq(users.username, identifier))).limit(1))[0];
}
...
export async function listUsers(search?: string) {
  ...
  if (!term) return db.select().from(users).orderBy(desc(users.createdAt));
  const pattern = `%${term}%`;
  return db.select().from(users).where(or(like(users.name, pattern), like(users.username, pattern), like(users.email, pattern), like(users.openId, pattern))).orderBy(desc(users.createdAt));
}
export async function getAdminStats() {
  const db = await getDb(); if (!db) return { users: 0, pendingDeposits: 0, pendingWithdrawals: 0 };
  const userRows = await db.select({ count: sql<number>`count(*)` }).from(users);
  const deposits = await db.select({ count: sql<number>`count(*)` }).from(depositRequests).where(eq(depositRequests.status, "pending"));
  const withdrawals = await db.select({ count: sql<number>`count(*)` }).from(withdrawalRequests).where(eq(withdrawalRequests.status, "pending"));
  return { users: Number(userRows[0]?.count ?? 0), pendingDeposits: Number(deposits[0]?.count ?? 0), pendingWithdrawals: Number(withdrawals[0]?.count ?? 0) };
}
...
export async function listDepositRequests() {
  const db = await getDb(); if (!db) return [];
  return db.select({ request: depositRequests, user: { id: users.id, name: users.name, email: users.email, username: users.username } }).from(depositRequests).leftJoin(users, eq(users.id, depositRequests.userId)).orderBy(desc(depositRequests.createdAt));
}
export async function listWithdrawalRequests() {
  const db = await getDb(); if (!db) return [];
  return db.select({ request: withdrawalRequests, user: { id: users.id, name: users.name, email: users.email, username: users.username } }).from(withdrawalRequests).leftJoin(users, eq(users.id, withdrawalRequests.userId)).orderBy(desc(withdrawalRequests.createdAt));
}
