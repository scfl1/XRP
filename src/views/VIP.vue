      ...
      <div class="shares-button-container">
        <button @click="goToShares" class="shares-button">
          <div class="shares-button-content">
            <span class="shares-icon">ℹ️</span>
            <div class="shares-text-container">
              <span class="shares-title">معلومات المنصة</span>
              <span class="shares-subtitle">Nova Demo</span>
            </div>
          </div>
          <span class="shares-badge">عرض تجريبي</span>
        </button>
      </div>
      ...
      <div v-else>
        ...
        <div v-if="userVip && userVip.level" class="current-vip-card" :class="{ 'elite-user': userVip.level >= 8 }">
          ...
          <div class="earnings-grid">
            <div class="earn-box">
              <span class="earn-label"><i class="fas fa-coins"></i> قيمة محاكاة</span>
              <span class="earn-value">{{ formatNumberEnglish(userVip.daily) }} <small>USDT</small></span>
            </div>
            <div class="earn-box">
              <span class="earn-label"><i class="fas fa-calendar-day"></i> محاكاة أسبوعية</span>
              <span class="earn-value">{{ formatNumberEnglish(userVip.daily * 7, 1) }} <small>USDT</small></span>
            </div>
            <div class="earn-box">
              <span class="earn-label"><i class="fas fa-calendar-week"></i> محاكاة شهرية</span>
              <span class="earn-value">{{ formatNumberEnglish(userVip.daily * 30, 0) }} <small>USDT</small></span>
            </div>
            <div class="earn-box">
              <span class="earn-label"><i class="fas fa-calendar-alt"></i> محاكاة سنوية</span>
            ...
            <div class="item-body">
              ...
              <div class="stats-grid-v2">
                <div class="mini-stat-v2" :class="{ 'elite-stat': plan.level >= 8 }">
                  <span class="stat-label-v2"><i class="fas fa-coins gold-icon"></i> قيمة محاكاة</span>
                  <span class="stat-value-v2">{{ formatNumberEnglish(plan.daily) }}</span>
                </div>
                <div class="mini-stat-v2" :class="{ 'elite-stat': plan.level >= 8 }">
                  <span class="stat-label-v2"><i class="fas fa-calendar-day gold-icon"></i> محاكاة أسبوعية</span>
                  <span class="stat-value-v2">{{ formatNumberEnglish(plan.daily * 7, 1) }}</span>
                </div>
                <div class="mini-stat-v2" :class="{ 'elite-stat': plan.level >= 8 }">
                  <span class="stat-label-v2"><i class="fas fa-calendar-week gold-icon"></i> محاكاة شهرية</span>
                  <span class="stat-value-v2">{{ formatNumberEnglish(plan.daily * 30, 0) }}</span>
                </div>
                <div class="mini-stat-v2" :class="{ 'elite-stat': plan.level >= 8 }">
                  <span class="stat-label-v2"><i class="fas fa-calendar-alt gold-icon"></i> محاكاة سنوية</span>
        ...
        <div class="elite-footer">
          <div class="elite-info-box">
            ...
            <div class="elite-info-content">
              <h4>🌟 برنامج Nova التجريبي 🌟</h4>
              <p>المستويات من VIP 8 إلى VIP 15 تتمتع بمزايا واجهة إضافية وتجربة استخدام محسّنة داخل النسخة التجريبية</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="modal-body">
            <div class="investment-text">
              ...
              <div class="section">
                <h4>💼 مميزات الخطة:</h4>
                <ul>
                  <li>لا توجد عوائد مالية تلقائية في النسخة التجريبية</li>
              ...
              <div class="section">
                <h4>🚀 آلية العمل:</h4>
                <p>عند تفعيل الباقة، هذه العضويات تجريبية لأغراض اختبار الواجهة، ولا تنشئ عوائد مالية تلقائية.</p>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      plans: [
        { level: 1, name: "VIP 1", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 2, name: "VIP 2", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 3, name: "VIP 3", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 4, name: "VIP 4", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 5, name: "VIP 5", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 6, name: "VIP 6", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 7, name: "VIP 7", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 8, name: "VIP 8", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 9, name: "VIP 9", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 10, name: "VIP 10", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 11, name: "VIP 11", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 12, name: "VIP 12", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 13, name: "VIP 13", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 14, name: "VIP 14", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
        { level: 15, name: "VIP 15", price: 0, tasks: 1, daily: 0, durationSeconds: 365 * 86400 },
      ],
  ...
  methods: {
    ...
    async executePurchase() {
      ...
      try {
        ...
        this.showSuccess(`تم تفعيل المستوى التجريبي VIP ${this.selectedPlan.level} بنجاح`);
        await this.init();
      } catch (err) {
    ...
    goToShares() {
      if (!this.userVip || this.userVip.level < 8) {
        this.showError('🔒 يجب تفعيل مستوى VIP 8 للوصول إلى معلومات المنصة');
        return;
      }
...
<style scoped>
.vip-page{background:transparent}.container{max-width:1000px}.page-title{font-weight:900;letter-spacing:-.5px}.vip-card-item,.current-vip-card,.shares-button-container{border-color:rgba(169,140,255,.18)!important}.vip-card-item{background:linear-gradient(145deg,#151a2b,#111628)!important}.filter-btn.active,.btn-details-white,.btn-upgrade-auto,.shares-button{background:linear-gradient(135deg,#7257ff,#9b7cff)!important}.gold-spinner{border-color:#7257ff transparent #a98cff transparent!important}
</style><style scoped>
/* NOVA DESIGN SYSTEM — VIP */
.vip-page{min-height:100vh!important;padding:24px 14px 110px!important;background:radial-gradient(circle at 10% 4%,rgba(49,196,255,.12),transparent 26%),radial-gradient(circle at 90% 40%,rgba(114,87,255,.16),transparent 30%),#070914!important;color:#fff!important}.container{max-width:1000px!important}.page-title{font-size:30px!important;color:#fff!important;font-weight:950!important}.title-glow{filter:none!important}.shares-button-container{margin:18px 0!important}.shares-button{border:1px solid rgba(169,140,255,.18)!important;background:linear-gradient(145deg,#151a31,#0f1427)!important;border-radius:20px!important;box-shadow:0 16px 36px rgba(0,0,0,.22)!important}.shares-title{color:#fff!important}.shares-subtitle{color:#8f9ab8!important}.shares-badge{background:rgba(49,196,255,.10)!important;color:#72d8ff!important;border:1px solid rgba(49,196,255,.18)!important}.current-vip-card,.vip-card-item{background:linear-gradient(145deg,#151a31,#0e1326)!important;border:1px solid rgba(169,140,255,.16)!important;border-radius:24px!important;box-shadow:0 18px 44px rgba(0,0,0,.22)!important}.vip-title{color:#fff!important}.earn-box{background:#0b1020!important;border-color:rgba(169,140,255,.10)!important}.earn-label{color:#8f9ab8!important}.earn-value{color:#a98cff!important}.remaining-timer,.last-reward-info{background:rgba(49,196,255,.06)!important;border-color:rgba(49,196,255,.12)!important;color:#aeb8d0!important}.filter-buttons{gap:8px!important}.filter-btn{background:#0b1020!important;color:#8f9ab8!important;border-color:rgba(169,140,255,.12)!important;border-radius:13px!important}.filter-btn.active,.btn-details-white,.btn-upgrade-auto,.shares-button{background:linear-gradient(135deg,#7257ff,#9b7cff)!important;color:#fff!important}.btn-details-white,.btn-upgrade-auto{border:0!important;border-radius:13px!important}.elite-ribbon{background:linear-gradient(135deg,#31c4ff,#7257ff)!important}.gold-spinner{border-color:#7257ff transparent #31c4ff transparent!important}
</style>
