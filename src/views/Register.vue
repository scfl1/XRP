    ...
    <main class="register-area">
      ...
      <div class="register-card">
        ...
        <div class="heading-section">
          ...
          <p>
            {{ translations[ currentLanguage ].registerSubtitle }}
          </p>
        ...
        <!-- =========================
             ...
             ========================= -->
        <label class="field-label">
          {{ translations[ currentLanguage ].inviteCodeLabel }}
        ...
        <div class="field-wrapper">
          ...
          <input
            type="text"
            v-model="inviteCode"
            :placeholder="translations[ currentLanguage ].inviteCodePlaceholder"
            class="input-field"
            autocomplete="off"
          />
...
import logo from "../assets/lumarise-logo.svg";
import logoBg from "../assets/lumarise-bg.svg";
...
export default {
    ...
    return {
      ...
      translations: {
        ar: {
          ...
          registerSubtitle: "انضم إلينا وابدأ رحلتك المالية",
          ...
          inviteCodeLabel: "كود الإحالة (اختياري)",
          inviteCodePlaceholder: "أدخل كود الإحالة إن وجد",
          registerButton: "إنشاء حساب",
          loadingText: "جارٍ إنشاء الحساب...",
          haveAccount: "لديك حساب بالفعل؟",
          loginLink: "تسجيل الدخول",
          errorInvalidEmail: "الرجاء إدخال بريد إلكتروني صحيح",
          errorPhoneExists: "رقم الهاتف مسجل مسبقاً",
          errorEmailExists: "البريد الإلكتروني مسجل مسبقاً",
          ...
          errorGeneral: "حدث خطأ أثناء إنشاء الحساب، يرجى المحاولة لاحقاً",
          errorPhoneInvalid: "رقم الهاتف غير صحيح"
        },
        en: {
          registerTitle: "Create Account",
          registerSubtitle: "Join us and start your financial journey",
          emailLabel: "Email",
          ...
          errorPhoneExists: "Phone number is already registered",
          ...
          errorGeneral: "An error occurred while creating the account, please try again later",
          errorPhoneInvalid: "Invalid phone number"
        },
        ru: {
          registerTitle: "Создать аккаунт",
          registerSubtitle: "Присоединяйтесь к нам и начните свой финансовый путь",
          ...
          inviteCodeLabel: "Код приглашения (опционально)",
          inviteCodePlaceholder: "Введите код приглашения, если есть",
          registerButton: "Создать аккаунт",
          loadingText: "Создание аккаунта...",
          haveAccount: "Уже есть аккаунт?",
          loginLink: "Войти",
          errorInvalidEmail: "Пожалуйста, введите действительный адрес электронной почты",
          errorPhoneExists: "Номер телефона уже зарегистрирован",
          errorEmailExists: "Электронная почта уже зарегистрирована",
          ...
          errorGeneral: "Произошла ошибка при создании аккаунта, пожалуйста, попробуйте позже",
          errorPhoneInvalid: "Неверный номер телефона"
        }
      }
    };
  },
  ...
  methods: {
    ...
    validatePhoneNumber() {
      ...
      if (!this.countryCode) {
        this.phoneError = this.translations[this.currentLanguage].errorPhoneCode;
        return false;
      }

      if (!this.phoneNumber) {
        this.phoneError = this.translations[this.currentLanguage].errorPhoneEmpty;
        return false;
      }

      const cleanPhone = this.phoneNumber.replace(/[^0-9]/g, '');
      if (cleanPhone.length < 7 || cleanPhone.length > 15) {
        this.phoneError = this.translations[this.currentLanguage].errorPhoneLength;
        return false;
      }
    ...
    async registerUser() {
      ...
      if (this.registerType === 'phone') {
        ...
        if (!this.validateEmail(this.email)) {
          this.errorMessage = this.translations[this.currentLanguage].errorInvalidEmail;
          return;
        }
      }
      ...
      if (!this.validatePassword(this.password)) {
        this.passwordError = this.translations[this.currentLanguage].errorPasswordLength;
        return;
      }

      if (this.password !== this.confirmPassword) {
        this.passwordError = this.translations[this.currentLanguage].errorPasswordMatch;
        return;
      }
      ...
      try {
        ...
        if (this.registerType === 'phone') {
          ...
          if (exists) {
            this.errorMessage = this.translations[this.currentLanguage].errorPhoneExists;
            this.loading = false;
            return;
          }
        }
        ...
        // إنشاء كود الإحالة الخاص بالمستخدم
        ...
        if (error.code === 'auth/email-already-in-use') {
          if (this.registerType === 'email') {
            this.errorMessage = this.translations[this.currentLanguage].errorEmailExists;
          } else {
            this.errorMessage = this.translations[this.currentLanguage].errorPhoneExists;
          }
        } else {
          this.errorMessage = this.translations[this.currentLanguage].errorGeneral;
        }
      } finally {
        this.loading = false;
      }
